...
function filter_schedules(future: boolean) {
  const schs = future
    ? calendar.schedule.filter((value) => value.tag === '千里眼')
    : calendar.schedule.filter((value) => value.tag !== '千里眼')
...
const sheet_height = ref(
  filter_schedules(props.future).length * (sc_height + sc_margin + 8) + sc_margin
)
</script>
<template>
  <div style="width: fit-content">
    ...
    <div style="overflow: hidden">
      <div>
        <div
          :style="{
            // height: `${sheet_height + sc_height}px`
        }"
          style="
            position: absolute;
            width: 2px;
        ...
        <div
          ...
          :style="{
            ...
            //   -96 +
            //   -96 *
            //     ((calendar.now.valueOf() -
            //       calendar.now.clone().hours(0).minutes(0).seconds(0).milliseconds(0).valueOf()) /
            //       86400000)
            // }px`
          }"
        >
          <div style="position: absolute; z-index: 1" :style="{ 'margin-top': `${sc_height}px` }">
            ...
            >
              ...
              <div
                style="height: 0"
                :style="{
                  // left: `${Math.max(96 * ((calendar.now - s.timeStart) / 86400000), 0)}px`
                }"
              >
                <div
                  style="
